...
Curated sample venue catalog, editorial example lists, and fictional diner profiles are marked demo. No claimed live Blackbird membership, availability, check-in, reward balance, or endorsement. Demo accounts can save places, curate lists and follow diners, but cannot review. Demo account data persists in D1 but the browser cookie is its only recovery mechanism; use a real account for cross-device ownership. Public URLs work across browsers. Production can turn demo account creation off.
