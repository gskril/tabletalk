...
2. **Join:** Connect with Blackbird uses server-side OAuth + PKCE. Public browsing never redirects to login. Blackbird is the only authentication method. When credentials are absent, explain that sign-in is unavailable and allow public browsing. Demo signup and ChatGPT authentication are removed; legacy sessions cannot authorize actions.
...
4. **Review/rank:** member writes a 1–10 score (one decimal), review, favorite dish and visit date. One review per member and venue; editing replaces it; deleting removes it from the feed and average. Personal rankings sort by rating. Creating or editing a review requires an imported check-in belonging to the authenticated Blackbird member at that exact location. The server checks member identity, location ID and provider environment. Demo/platform accounts cannot post reviews. Only reviews with this proof appear publicly or contribute to ratings; legacy unverified rows remain stored but hidden. Verification concerns the location visit; the chosen review date is a personal note.
...
7. **Passport:** signing in automatically starts a private visit sync when the authenticated page loads. Show syncing, ready, error and expired-access states; offer only Retry sync or Reconnect when recovery is needed. While access is valid, page visits refresh history after 15 minutes. A per-member database lease prevents duplicate imports. Check-ins remain private; the user chooses which reviews to publish.
...
Curated sample venue catalog, editorial example lists, and fictional diner profiles are marked demo. No claimed live Blackbird membership, availability, check-in, reward balance, or endorsement. No demo account creation is offered. Existing sample profiles/lists remain clearly labeled sample content. All authenticated actions require Blackbird; public URLs work across browsers.
