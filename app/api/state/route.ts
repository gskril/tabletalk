...
export async function GET(request?: Request) {
  ...
  try {
    ...
    const includeCatalog = !request || new URL(request.url).searchParams.get("catalog") !== "separate";
    ...
    const [
      ...
      query(`SELECT id,name,bio,color,avatar,demo FROM profiles WHERE id IN (${memberProfileIds})`),
      query(
      ...
      query("SELECT b.venue_id FROM bookmarks b JOIN venues v ON v.id=b.venue_id WHERE b.user_id=? AND v.source!='demo'", uid),
      query(`SELECT target_id FROM follows WHERE user_id=? AND target_id IN (${memberProfileIds})`, uid),
      ...
      ...(includeCatalog ? [query("SELECT * FROM venues WHERE source!='demo' ORDER BY source DESC,name")] : []),
    ])).map(result => result.results);
    const people = peopleRows as unknown as Person[];
    const visitCounts = new Map<string, number>();
    for (const visit of publicVisits)
      visitCounts.set(String(visit.user_id), (visitCounts.get(String(visit.user_id)) || 0) + 1);
    ...
    return Response.json(
      ...
      { headers: { "Cache-Control": "private, no-store", "Server-Timing": `state;dur=${(performance.now() - started).toFixed(1)}` } },
    );
  } catch (e) {
    return failure(e);
  }
